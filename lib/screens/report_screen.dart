...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final topInfoStyle = const TextStyle(fontSize: 16, fontWeight: FontWeight.bold);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Отчёт Жана Статистика'),
      ),
      body: Padding(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text('Жалпы эффективдүүлүк: ${_overallEfficiency()}', style: topInfoStyle),
                    const SizedBox(height: 8),
                    Text('Топ модель: ${_mostFrequentModel()}', style: topInfoStyle),
                  ],
                ),
              ),
            ),
            const SizedBox(height: 24),
            const Text('Күнүмдүк Кройлор (Отчёт):', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ...
                : ListView(
                    children: _cutsPerDay().entries.map((e) {
                      return ListTile(
                        ...
                        title: Text('Дата: ${e.key}', style: const TextStyle(fontWeight: FontWeight.bold)),
                        trailing: Text(
                          '${e.value} крой', 
